...
class QuestionState {
  ...
  const QuestionState({
    this.banks = const [],
    this.selectedBankUid,
    this.mixMode = false,
    this.isDirty = false,
  });

  QuestionBank? get selectedBank =>
      selectedBankUid == null
          ? null
          : banks.cast<QuestionBank?>().firstWhere(
            (b) => b!.uid == selectedBankUid,
            orElse: () => null,
          );
  List<QuestionBank> get mixedBanks =>
      mixMode ? banks : (selectedBank != null ? [selectedBank!] : []);
  List<Question> get allQuestions =>
      mixedBanks.expand((b) => b.questions).toList();

  QuestionState copyWith({
    List<QuestionBank>? banks,
    String? selectedBankUid,
    bool? mixMode,
    bool? isDirty,
  }) => QuestionState(
    banks: banks ?? this.banks,
    selectedBankUid: selectedBankUid ?? this.selectedBankUid,
...
class QuestionNotifier extends StateNotifier<QuestionState> {
  ...
  void loadFromData(List<QuestionBank> banks) =>
      state = state.copyWith(banks: banks, isDirty: false);
  void addBank(QuestionBank bank) =>
      state = state.copyWith(
        banks: [...state.banks, bank],
        selectedBankUid: bank.uid,
        isDirty: true,
      );
