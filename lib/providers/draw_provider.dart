...
class DrawState {
  ...
  const DrawState({this.noReplacement = false, this.lockedGroupUid,
      this.removedMembers = const [], this.removedGroups = const []});

  DrawState copyWith({bool? noReplacement, String? lockedGroupUid,
      List<String>? removedMembers, List<String>? removedGroups, bool clearLocked = false}) =>
      DrawState(noReplacement: noReplacement ?? this.noReplacement,
...
class DrawNotifier extends StateNotifier<DrawState> {
  ...
  List<Member> get availableMembers {
    final cls = _class; if (cls == null) return [];
    List<Member> pool = state.lockedGroupUid != null
        ? (cls.groups.cast<Group?>().firstWhere((g) => g!.uid == state.lockedGroupUid, orElse: () => null)?.members ?? [])
        : cls.allMembers;
  ...
  Member? drawMember() { final pool = availableMembers; if (pool.isEmpty) return null;
    final r = pool[Random().nextInt(pool.length)];
    if (state.noReplacement) addRemovedMember(r.uid); return r; }

  Group? drawGroup() { final pool = availableGroups; if (pool.isEmpty) return null;
    final r = pool[Random().nextInt(pool.length)];
    if (state.noReplacement) addRemovedGroup(r.uid); return r; }
}
