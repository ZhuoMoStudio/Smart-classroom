...
class SyncNotifier extends StateNotifier<SyncState> {
  ...
  void syncComplete() =>
      state = state.copyWith(status: SyncStatus.online, message: '同步完成', progress: 1.0,
          lastSyncTime: DateTime.now().toIso8601String());

  void syncError(String error) => state = state.copyWith(
      status: SyncStatus.error, message: error, recentErrors: [...state.recentErrors, error]);

  void setStatus(SyncStatus status) => state = state.copyWith(status: status);
}
