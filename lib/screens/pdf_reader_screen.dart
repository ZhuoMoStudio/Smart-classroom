...
class PdfReaderScreen extends ConsumerStatefulWidget {
  ...
  const PdfReaderScreen({
    super.key, this.title, this.initialPage = 1,
    this.networkUrl, this.localFilePath,
  });
  @override
...
class _State extends ConsumerState<PdfReaderScreen> {
    ...
    try {
      ...
      if (widget.localFilePath != null) {
        ...
        if (cached != null) { fp = cached; _status = '从缓存加载'; }
          ...
          _status = '下载中...';
          fp = await cm.downloadAndCache(widget.networkUrl!,
            onProgress: (r, t) { if (mounted && t > 0) setState(() => _progress = r / t); },
            onStatus: (s) { if (mounted) setState(() {
              switch (s) {
                case DownloadState.downloading: _status = '下载中...';
                case DownloadState.completed: _status = '下载完成';
                case DownloadState.failed: _status = '下载失败';
                default: break;
              }
            }); },
          );
      ...
      _fp = fp; _fileBytes = await File(fp!).readAsBytes();
  ...
  void _goPage() {
    ...
    _ctrl.goToPage(pageNumber: p); _pageInput.clear(); FocusScope.of(context).unfocus();
    setState(() => _cur = p); _resetHide();
  }
  ...
  Widget build(BuildContext ctx) {
    return Scaffold(
      ...
      body: Stack(children: [
        if (_loading) _loadingView(ctx)
        else if (_fp != null && _fileBytes != null) _pdfView()
        else _errorView(ctx),
        if (_showCtrl && !_loading) _topBar(ctx),
        if (_showCtrl && !_loading) _bottomBar(ctx),
        if (!_showCtrl && !_loading)
          Positioned.fill(child: GestureDetector(behavior: HitTestBehavior.translucent, onTap: _resetHide)),
      ]),
    );
  }
      ...
      params: PdfViewerParams(
        ...
        onViewerReady: (doc, ctrl) {
          WidgetsBinding.instance.addPostFrameCallback((_) {
            if (mounted) {
              setState(() { _total = doc.pages.length; _cur = ctrl.pageNumber ?? widget.initialPage; });
    ...
    child: AnimatedOpacity(
      ...
      child: Container(
        decoration: const BoxDecoration(
          gradient: LinearGradient(colors: [Colors.black87, Colors.black54, Colors.transparent],
        ...
        child: Row(children: [
          ...
          Expanded(child: Text(widget.title ?? '教材阅读',
  ...
  Widget _bottomBar(BuildContext ctx) => Positioned(
    ...
    child: AnimatedOpacity(
      ...
      child: Container(
        decoration: const BoxDecoration(
          gradient: LinearGradient(colors: [Colors.transparent, Colors.black54, Colors.black87],
              begin: Alignment.topCenter, end: Alignment.bottomCenter),
        ),
          ...
          GestureDetector(
            onTap: () {
              ...
              showDialog(context: ctx, builder: (c) => AlertDialog(
                ...
                content: TextField(autofocus: true, keyboardType: TextInputType.number,
  ...
  Widget _loadingView(BuildContext ctx) => Center(child: Column(mainAxisSize: MainAxisSize.min, children: [
    ...
    TextButton(onPressed: () => Navigator.pop(ctx), child: const Text('取消返回', style: TextStyle(color: Colors.white54))),
  ]));
