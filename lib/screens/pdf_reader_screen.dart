...
/// 分卷合并输出子目录名
const _mergeOutputDirName = '.merged';
...
class _State extends ConsumerState<PdfReaderScreen> {
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      if (widget.localFilePath != null) {
        ...
        // 分卷合并到 .merged 子目录
        await PdfMergeService.mergeAllInDirectory(
          Directory(fp).parent.path,
          outputDirName: _mergeOutputDirName,
        );
        // 检测是否有合并后的文件
        final mergedParent =
            Directory('${Directory(fp).parent.path}/$_mergeOutputDirName');
        final mergedName =
            '${p.basenameWithoutExtension(widget.networkUrl!.split('/').last)}.pdf';
        final mergedFile = File('${mergedParent.path}/$mergedName');
        if (await mergedFile.exists()) {
          fp = mergedFile.path;
        }
      }
      if (fp == null || !await File(fp).exists()) {
        throw Exception('无法加载');
      }
      _fp = fp;
      // pdfrx ^1.0.100 没有 PdfViewer.file，使用 PdfViewer.data 兼容
  ...
  Widget _pdfView() {
    // pdfrx ^1.0.100 没有 PdfViewer.file，使用 PdfViewer.data
          ...
          Row(
            ...
            children: [
              ...
              OutlinedButton.icon(
                ...
                label:
                    const Text('本地文件', style: TextStyle(color: Colors.white70)),
                onPressed: _openLocal,
              ),
            ],
          ),
