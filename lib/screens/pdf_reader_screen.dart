...
class PdfReaderScreen extends ConsumerStatefulWidget {
  ...
  @override ConsumerState<PdfReaderScreen> createState() => _PdfReaderScreenState();
}
...
class _PdfReaderScreenState extends ConsumerState<PdfReaderScreen> {
  ...
  @override void dispose() { _ctrl.dispose(); _pageInput.dispose(); _hideTimer?.cancel(); super.dispose(); }
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      if (widget.localFilePath != null) {
        ...
        if (cached != null) {
          fp = cached; _status = '从缓存加载';
        } else {
          _status = '下载中（加速节点）...';
          fp = await cm.downloadAndCache(widget.networkUrl!, onProgress: (r, t) {
            if (mounted && t > 0) setState(() { _progress = r / t; _status = '下载 ${(r/1048576).toStringAsFixed(1)} MB'; });
          }, onStatus: (s) {
            if (mounted) setState(() {
              switch (s) { case DownloadState.downloading: _status='下载中...'; case DownloadState.completed: _status='完成...'; case DownloadState.failed: _status='失败'; default: break; }
            });
          });
        }
  ...
  Widget _pdfView() => GestureDetector(onTap: _resetHide, child: PdfViewer.file(_fp!, controller: _ctrl, initialPageNumber: widget.initialPage, params: const PdfViewerParams(padding: 8, scrollByMouseWheel: true), onViewerReady: (doc, ctrl) { WidgetsBinding.instance.addPostFrameCallback((_) { if (mounted) { setState(() { _total = doc.pages.length; _cur = ctrl.pageNumber ?? widget.initialPage; }); if (widget.initialPage > 1) ctrl.goToPage(pageNumber: widget.initialPage); _resetHide(); } }); }, onPageChanged: (pn) { if (mounted) setState(() => _cur = pn ?? 1); }));
