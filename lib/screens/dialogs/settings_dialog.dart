  ...
  Widget build(BuildContext context) {
    return AlertDialog(
        ...
        child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Row(children: [
            Expanded(child: DropdownButtonFormField<String?>(
              value: _local.currentGrade, isDense: true,
              decoration: const InputDecoration(labelText: '年级', hintText: '选择年级'),
              items: [const DropdownMenuItem<String?>(value: null, child: Text('不限')), ..._grades.map((g) => DropdownMenuItem(value: g, child: Text(g)))],
              onChanged: (v) => setState(() => _local = _local.copyWith(currentGrade: v)),
            )),
            const SizedBox(width: 8),
            Expanded(child: DropdownButtonFormField<String?>(
              value: _local.currentSubject, isDense: true,
              decoration: const InputDecoration(labelText: '学科', hintText: '选择学科'),
              items: [const DropdownMenuItem<String?>(value: null, child: Text('不限')), ..._subjects.map((s) => DropdownMenuItem(value: s, child: Text(s)))],
          ...
          SwitchListTile(title: const Text('音效'), value: _local.soundEnabled, dense: true, onChanged: (v) => setState(() => _local = _local.copyWith(soundEnabled: v))),
          DropdownButtonFormField<String>(value: _local.layoutMode, decoration: const InputDecoration(labelText: '布局', isDense: true), items: const [
            DropdownMenuItem(value: 'auto', child: Text('自动')), DropdownMenuItem(value: 'landscape', child: Text('横屏')), DropdownMenuItem(value: 'portrait', child: Text('竖屏')),
          ], onChanged: (v) => setState(() => _local = _local.copyWith(layoutMode: v!))),
          TextFormField(initialValue: _local.timerPresets.join(','), decoration: const InputDecoration(labelText: '计时预设 (逗号分隔)', isDense: true),
          ...
          Row(children: [
            ...
            TextButton.icon(icon: const Icon(Icons.open_in_new, size: 14), label: const Text('注册账号'),
              onPressed: () => launchUrl(Uri.parse('https://www.jianguoyun.com/signup'), mode: LaunchMode.externalApplication)),
          ]),
          DropdownButtonFormField<String>(value: _local.cloudServiceType, decoration: const InputDecoration(labelText: '云服务', isDense: true),
            items: cloudPresets.map((p) => DropdownMenuItem(value: p.name, child: Text(p.name))).toList(),
            onChanged: (v) { final ps = cloudPresets.firstWhere((p) => p.name == v); setState(() => _local = _local.copyWith(cloudServiceType: v!, webdavUrl: ps.defaultUrl)); }),
          TextFormField(initialValue: _local.webdavUrl, decoration: const InputDecoration(labelText: 'WebDAV 地址', isDense: true, hintText: 'https://dav.jianguoyun.com/dav/'),
          ...
          Text('文件名禁止包含 \\ / : * ? " < > | 等特殊字符', style: TextStyle(fontSize: 11, color: Colors.grey.shade600)),
          const SizedBox(height: 8),
          Row(children: [
            ElevatedButton.icon(icon: const Icon(Icons.wifi, size: 16), label: const Text('测试连接'), onPressed: () async {
          ...
          DropdownButtonFormField<int>(value: _local.autoSyncInterval, decoration: const InputDecoration(labelText: '同步间隔', isDense: true), items: const [
            DropdownMenuItem(value: 0, child: Text('手动')), DropdownMenuItem(value: 5, child: Text('5分钟')), DropdownMenuItem(value: 15, child: Text('15分钟')), DropdownMenuItem(value: 30, child: Text('30分钟')),
          ...
          DropdownButtonFormField<int>(value: _local.autoSaveInterval, decoration: const InputDecoration(labelText: '保存间隔', isDense: true), items: const [
            DropdownMenuItem(value: 15, child: Text('15秒')), DropdownMenuItem(value: 30, child: Text('30秒')), DropdownMenuItem(value: 60, child: Text('60秒')), DropdownMenuItem(value: 120, child: Text('120秒')),
          ], onChanged: (v) => setState(() => _local = _local.copyWith(autoSaveInterval: v!))),
          TextFormField(initialValue: _local.usbDataPath ?? '', decoration: const InputDecoration(labelText: 'U盘数据路径', isDense: true, hintText: '留空自动检测'),
          ...
          Row(children: [
            ...
            TextButton.icon(icon: const Icon(Icons.favorite, size: 14), label: const Text('开源说明'), onPressed: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const OpenSourceScreen()))),
          ]),
          ElevatedButton.icon(icon: const Icon(Icons.system_update, size: 16), label: const Text('检查更新'), onPressed: () async {
            final r = await UpdateService.check();
            if (!mounted) return;
            if (r.hasUpdate) {
              final ok = await showDialog<bool>(context: context, builder: (c) => AlertDialog(
                title: const Text('发现新版本'), content: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
                  Text('当前版本: v${r.currentVersion}'), Text('最新版本: v${r.latestVersion}'), const SizedBox(height: 8), const Text('是否前往下载页面？'),
                ]), actions: [TextButton(onPressed: () => Navigator.pop(c, false), child: const Text('取消')), FilledButton(onPressed: () => Navigator.pop(c, true), child: const Text('去下载'))],
              ));
      ...
      actions: [
        ...
        FilledButton(onPressed: () { ref.read(settingsProvider.notifier).update(_local); AudioEngine().setEnabled(_local.soundEnabled); Navigator.pop(context); }, child: const Text('应用')),
      ],
    );
  }

  Widget _section(String t) => Padding(padding: const EdgeInsets.only(top: 4, bottom: 4), child: Text(t, style: TextStyle(fontWeight: FontWeight.bold, color: Theme.of(context).colorScheme.primary, fontSize: 14)));
