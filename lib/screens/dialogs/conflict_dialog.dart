...
class ConflictDialog extends StatelessWidget {
  ...
  Widget build(BuildContext ctx) => AlertDialog(
    ...
    content: Column(mainAxisSize: MainAxisSize.min, children: [
      Text('文件 "${conflict.itemName}" 存在冲突'), const SizedBox(height: 8),
    ...
    actions: [
      TextButton(onPressed: () => Navigator.pop(ctx, 'local'), child: const Text('保留本地')),
      TextButton(onPressed: () => Navigator.pop(ctx, 'remote'), child: const Text('保留云端')),
      TextButton(onPressed: () => Navigator.pop(ctx, 'cancel'), child: const Text('跳过')),
    ],
  );
}
