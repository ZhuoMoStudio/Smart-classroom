...
class QuestionPanel extends ConsumerWidget {
          ...
          child: ListView(
            ...
            children: [
              FilterChip(
                ...
                onSelected: (_) =>
                    ref.read(questionProvider.notifier).selectBank(null),
              ),
              ...
              ...qs.banks.map((bank) {
                ...
                return Padding(
                  ...
                  child: ChoiceChip(
                              ...
                              showDialog(
                                ...
                                builder: (ctx) => AlertDialog(
                                  ...
                                  actions: [
                                    ...
                                    FilledButton(
                                        onPressed: () {
                                          final updated = qs.banks
                                              .where((b) => b.uid != bank.uid)
                                              .toList();
                                          ref
                    ...
                    selected: selected,
                    onSelected: (_) =>
                        ref.read(questionProvider.notifier).selectBank(bank.uid),
                  ),
                );
              }),
              ...
              Row(
                ...
                children: [
                  ...
                  Checkbox(
                    value: qs.mixMode,
                    onChanged: (_) =>
                        ref.read(questionProvider.notifier).toggleMixMode(),
  ...
  Widget _grid(
      QuestionState qs, ThemeData t, WidgetRef ref, BuildContext ctx) {
      ...
      builder: (ctx, cs) {
        ...
        return GridView.builder(
          ...
          itemBuilder: (ctx, i) {
            ...
            if (q.used) {
              bg = t.brightness == Brightness.light
            ...
            if (q.answer != null && q.answer!.isNotEmpty) {
              bottom = const BorderSide(color: Colors.green, width: 3);
            }
              ...
              child: Container(
                decoration: BoxDecoration(
                  ...
                  border: Border(
                    ...
                    top: BorderSide(
                        color: t.colorScheme.outline.withOpacity(0.15)),
                    right: BorderSide(
                        color: t.colorScheme.outline.withOpacity(0.15)),
                  ),
                ),
        ...
        children: [
          Row(
            ...
            children: [
              ...
              const SizedBox(width: 4),
              Text(
                  '风险题: ${qlist.where((q) => q.isRisk).length}',
                  style: const TextStyle(fontSize: 12)),
              const SizedBox(width: 16),
              const Icon(Icons.check_circle, size: 14, color: Colors.green),
              const SizedBox(width: 4),
              Text('已答: ${qlist.where((q) => q.used).length} / ${qlist.length}',
                  style: const TextStyle(fontSize: 12)),
            ],
          ),
          TextButton.icon(
            icon: const Icon(Icons.refresh, size: 14),
            label: const Text('重置已答', style: TextStyle(fontSize: 12)),
            ...
            onPressed: () {
              ...
              if (b != null) {
                ...
                ToastOverlay.show(ctx, '已重置题库: ${b.name}');
              } else {
                ToastOverlay.show(ctx, '请先选择一个题库');
              }
            },
          ),
        ],
  ...
  void _detail(BuildContext ctx, Question q, WidgetRef ref) {
    showDialog<bool>(context: ctx, builder: (_) => QuestionDetailDialog(question: q))
        .then((marked) {
      if (marked == true) {
  ...
  Future<void> _import(WidgetRef ref, BuildContext ctx) async {
    ...
    if (p == null) return;
    try {
      final bn = p.split(Platform.pathSeparator).last
          .replaceAll(RegExp(r'\.(xlsx|xls)$'), '');
      final bank = await ExcelService.parseQuestionBank(p, bn);
      ref.read(questionProvider.notifier).addBank(bank);
      ToastOverlay.show(
          ctx, '导入成功: ${bank.name} (${bank.questions.length} 题)',
          type: ToastType.success);
    } catch (e) {
      ToastOverlay.show(ctx, '导入失败: $e', type: ToastType.error);
    }
  }
