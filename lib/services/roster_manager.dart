...
/// 与旧实现的差异（刻意的）
/// ----------------------
/// 1. 旧实现只支持阿拉伯数字班级，但中国学校更常见的是「三年二班」。这里把中文数字
...
class RosterManager {
  ...
  static (String?, String?) extractGradeClass(String fileName) {
    ...
    // 年级部分。备选顺序很重要：更具体的写在前面。
    // 否则「高一年级」会被「一年级」抢先匹配掉、「高」被丢掉，
    // 进而把「高一年级3班」和「一年级3班」错误地归为同一组。
    const grade = r'(高[一二三]|初[一二三]|小[一二三四五六]'
        r'|(?:高|初)?[0-9]+年级'
        r'|(?:高|初)?[一二三四五六七八九十]+年级'
        // 「初中二年级 / 高中一年级」这类中间夹了「中」的写法必须单独列出。
        // 否则：对于「初中二年级2班」，前面几个备选在位置 0 全部失败，
        // 正则引擎前进到「二年级」处才匹配成功 —— 前缀「初」就这样被丢掉，
        // 使「初中二年级2班」与「二年级2班」归一到同一个分组键。
        // 而同一分组键意味着其中一份会被当作旧名单删掉，也就是删错班。
        // （这条是被 flutter test 抓出来的：我的 Python 移植版当时漏了对应的用例。）
        r'|(?:高|初|小)中[0-9]+年级'
        r'|(?:高|初|小)中[一二三四五六七八九十]+年级'
        r'|[0-9]+年'
        r'|[一二三四五六七八九十]+年)';
