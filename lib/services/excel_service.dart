...
class ExcelService {
  // ========== 题库导入 ==========
  static Future<QuestionBank> parseQuestionBank(String filePath, String bankName) async {
    ...
    for (int i = 0; i < table.maxRows; i++) {
      if (i == 0) {
        ...
        if (fc.contains('题目')||fc.contains('问题')||fc.contains('Question')||fc.contains('题干')) continue;
      }
      ...
      final isRisk = riskStr=='是'||riskStr=='y'||riskStr=='yes'||riskStr=='1'||riskStr=='true'||riskStr=='√'||riskStr=='✓';
      questions.add(Question(uid:_uid(), index:qi, text:text, answer:answer.isNotEmpty?answer:null, isRisk:isRisk));
    }
  ...
  static Future<File> exportScores(List<Classroom> classrooms, String outputPath) async {
    final excel = Excel.createExcel();
    final sheet = excel['积分数据'];
    sheet.appendRow(<dynamic>['班级','小组','姓名','积分','段位']);
    for (final c in classrooms) {
      for (final g in c.groups) {
        final sorted = List<Member>.from(g.members)..sort((a,b)=>b.score.compareTo(a.score));
  ...
  // ========== 积分导入 ==========
  static Future<Map<String, Map<String, Map<String, double>>>> importScores(String filePath) async {
    final bytes = await File(filePath).readAsBytes();
    final excel = Excel.decodeBytes(bytes);
    final sheet = excel.tables.keys.first;
    final table = excel.tables[sheet]!;
    ...
    for (int i = 1; i < table.maxRows; i++) {
      final cn=_cell(table,i,0).trim(), gn=_cell(table,i,1).trim(), mn=_cell(table,i,2).trim(), ss=_cell(table,i,3).trim();
