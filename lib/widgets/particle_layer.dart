...
class ParticleLayerState extends State<ParticleLayer> with SingleTickerProviderStateMixin {
  final _ps = <_Particle>[]; late AnimationController _c;
  @override void initState() { super.initState();
    _c = AnimationController(duration: const Duration(seconds: 1), vsync: this)..addListener(() => setState(() {})); }
  void emit(Offset p) { final r = Random();
    for (int i=0;i<12;i++) { final a = r.nextDouble()*2*pi, sp = 80+r.nextDouble()*120;
      _ps.add(_Particle(x:p.dx,y:p.dy,vx:cos(a)*sp,vy:sin(a)*sp,color:Colors.primaries[r.nextInt(Colors.primaries.length)])); }
    _c.forward(from: 0); }
  @override void dispose() { _c.dispose(); super.dispose(); }
  @override Widget build(BuildContext ctx) => Stack(children: [widget.child,
    Positioned.fill(child: IgnorePointer(child: CustomPaint(painter: _ParticlePainter(particles:_ps,progress:_c.value))))]);
}
